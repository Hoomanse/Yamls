...
def wordPopulate(template, out, property_address, pdfFileProperties, improvement, property):
    ...
    pagesTotalNumber = 1
    pdfFileProperties['tableContent'].update({pageNumber: 'Remodeling Detailed Assumptions'})
...
def main(pdfFileProperties, improvement, property):
    ...
    wordPopulate(f'{templateLocation}remodelingAssumptionTemplate', f'{outLocation}remodelingAssumption{id}',
                 property_address, pdfFileProperties, improvement, property)
    word_to_pdf(f'{outLocation}remodelingAssumption{id}')
    maregePdf(f'{outLocation}remodelingAssumption{id}', f'{outLocation}out{id}')
