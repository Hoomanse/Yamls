...
def wordPopulate(template, out, property_address, pdfFileProperties, primary):
    pageNumber = pdfFileProperties['pageStart']
    pagesTotalNumber = 1
    pdfFileProperties['tableContent'].update({pageNumber: 'Financial Projection Summary – Primary Residence'})
    ...
    hp = primary['genericInput']['holdingPeriodAnnual']
    yr = {1: '-', 2: '-', 3: '-', 4: '-', 5: '-'}
    income = {1: '-', 2: '-', 3: '-', 4: '-', 5: '-'}
    oe = {1: '-', 2: '-', 3: '-', 4: '-', 5: '-'}
    loan = {1: '-', 2: '-', 3: '-', 4: '-', 5: '-'}
    ...
    for item in range(len(arrays)):
        ...
        income[arrays[item]] = fin_value(primary['financialProjectionSummary']['income'][item + 1])
        oe[arrays[item]] = fin_value(primary['financialProjectionSummary']['operationExpenses'][item + 1])
        loan[arrays[item]] = fin_value(primary['financialProjectionSummary']['loanPayments'][item + 1])
        itax[arrays[item]] = fin_value(primary['financialProjectionSummary']['incomeTax'][item + 1])
        opcash[arrays[item]] = fin_value(primary['financialProjectionSummary']['operationNetCashFlow'][item + 1])
    document.merge(ADR=property_address,
                   HP=str(hp), IC=fin_value(primary['initialCash'] * -1), YR1=yr[1], YR2=yr[2], YR3=yr[3], YR4=yr[4],
                   YR5=yr[5],
                   ...
                   NSP=fin_value(primary['netSalePrice']), LB=fin_value(-1 * primary['mortgageBalanceRemaining']),
                   TDS=fin_value(primary['taxProjection']['totalTaxDue']),
                   TSP=fin_value(primary['afterTaxEquityReversion']),
                   ATP=fin_value(2 * round(primary['beforeTaxNetProfit'] / 2000))
                   )
...
def figuresOnPDF(file, primary):
    befortax = primary["beforeTaxProfit"]
    aftertax = primary["afterTaxProfit"]
    afterTax_profit = fin_value(2 * round(primary['beforeTaxNetProfit'] / 2000))
    holding_period = primary['genericInput']['holdingPeriodAnnual']
...
def main(pdfFileProperties):
    ...
    wordPopulate(f'{templateLocation}primarySummaryTemplate', f'{outLocation}primarySummary{id}', property_address,
                 pdfFileProperties, primary)
    word_to_pdf(f'{outLocation}primarySummary{id}')
    figuresOnPDF(f'{outLocation}primarySummary{id}.png', primary)
    overlay(f'{outLocation}primarySummary{id}', f'{outLocation}primarySummary{id}.png',
            f'{outLocation}primarySummaryOut{id}')
    maregePdf(f'{outLocation}primarySummaryOut{id}', f'{outLocation}out{id}')
