...
def whatif(criteria, range, whatif_json):
    if range == 'low':
        return fin_value(min(whatif_json[criteria]['privateUse']['beforeTaxNetProfit']['low'],
                             whatif_json[criteria]['privateUse']['beforeTaxNetProfit']['high']))
    else:
        return fin_value(max(whatif_json[criteria]['privateUse']['beforeTaxNetProfit']['low'],
                             whatif_json[criteria]['privateUse']['beforeTaxNetProfit']['high']))
...
def wordPopulate(template, out, property_address, pdfFileProperties, primary, sensitivity, whatif_json):
    pageNumber = pdfFileProperties['pageStart']
    pagesTotalNumber = 1
    pdfFileProperties['tableContent'].update({pageNumber: 'What if Analysis – Primary Residence'})
    ...
    cust_1 = {'ADR': property_address,
              ...
              'PPIN': lowValue(primary['genericInput']['purchasePrice'], sensitivity, 'f'),
              'PP': fin_value(primary['genericInput']['purchasePrice']),
              'PPAX': highValue(primary['genericInput']['purchasePrice'], sensitivity, 'f'),
              'PPVI': whatif('purchasePrice', 'low', whatif_json),
              'PPV': fin_value(primary['beforeTaxNetProfit']),
              'PPVX': whatif('purchasePrice', 'high', whatif_json),

              ## what if Rental Income
              'RMIN': lowValue(primary['financialProjectionSummary']['income'][1] / 12, sensitivity, 'f'),
              'R': fin_value(primary['financialProjectionSummary']['income'][1] / 12),
              'RMAX': highValue(primary['financialProjectionSummary']['income'][1] / 12, sensitivity, 'f'),
              'RPVI': whatif('incomeAvoidedCosts', 'low', whatif_json),
              'RPVX': whatif('incomeAvoidedCosts', 'high', whatif_json),

              ## what if Opertaion Expense
              'OMIN': lowValue(primary['financialProjectionSummary']['operationExpenses'][1] / -12, sensitivity, 'f'),
              'O': fin_value(primary['financialProjectionSummary']['operationExpenses'][1] / -12),
              'OMAX': highValue(primary['financialProjectionSummary']['operationExpenses'][1] / -12, sensitivity, 'f'),
              'OPVI': whatif('operationExpenses', 'low', whatif_json),
              'OPVX': whatif('operationExpenses', 'high', whatif_json),

              ## what if SELLING PRICE
              'SMIN': lowValue(primary['taxProjection']['estimatedSalePrice'], sensitivity, 'f'),
              'S': fin_value(primary['taxProjection']['estimatedSalePrice']),
              'SMAX': highValue(primary['taxProjection']['estimatedSalePrice'], sensitivity, 'f'),
              'SPVI': whatif('SellPrice', 'low', whatif_json),
              'SPVX': whatif('SellPrice', 'high', whatif_json),

              ## what if Holding Period
              'HMIN': lowValue(primary['genericInput']['holdingPeriodAnnual'] * 12, sensitivity, 't'),
              'H': str(round(primary['genericInput']['holdingPeriodAnnual'])),
              'HMAX': highValue(primary['genericInput']['holdingPeriodAnnual'] * 12, sensitivity, 't'),
              'HPVI': whatif('holdingPeriodAnnual', 'low', whatif_json),
              'HPVX': whatif('holdingPeriodAnnual', 'high', whatif_json),

              ## what if Buy&SellExpense
              'BMIN': lowValue(primary['sellingExpense'] + primary['purchaseExpense'], sensitivity, 'f'),
              'B': fin_value(primary['sellingExpense'] + primary['purchaseExpense']),
              'BMAX': highValue(primary['sellingExpense'] + primary['purchaseExpense'], sensitivity, 'f'),
              'BPVI': whatif('transActionCost', 'low', whatif_json),
              'BPVX': whatif('transActionCost', 'high', whatif_json),

              ## what if DownPayment
              'DMIN': lowValue(primary['genericInput']['downPaymentRate'] * primary['genericInput']['purchasePrice'],
                               sensitivity, 'f'),
              'D': fin_value(primary['genericInput']['downPaymentRate'] * primary['genericInput']['purchasePrice']),
              'DMAX': highValue(primary['genericInput']['downPaymentRate'] * primary['genericInput']['purchasePrice'],
                                sensitivity, 'f'),
    ...
    for item in scenarios:
        npv = primary['beforeTaxNetProfit']
        lownpv = whatif_json[item]['privateUse']['beforeTaxNetProfit']['low']
        highnpv = whatif_json[item]['privateUse']['beforeTaxNetProfit']['high']
...
def main(pdfFileProperties):
    primary, whatif_json = initialize()
    sensitivity = 0.15
    ...
    wordPopulate(f'{templateLocation}primaryWhatIfTemplate', f'{outLocation}primaryWhatIf{id}', property_address,
                 pdfFileProperties, primary, sensitivity, whatif_json)
    word_to_pdf(f'{outLocation}primaryWhatIf{id}')
    figuresOnPDF(f'{outLocation}primaryWhatIf{id}')
    overlay(f'{outLocation}primaryWhatIf{id}', f'{outLocation}primaryWhatIf{id}.png',
            f'{outLocation}primaryWhatIfOut{id}')
    maregePdf(f'{outLocation}primaryWhatIfOut{id}', f'{outLocation}out{id}')
