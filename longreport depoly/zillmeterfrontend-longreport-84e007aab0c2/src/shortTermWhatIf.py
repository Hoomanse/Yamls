...
def whatif(criteria, range, whatif_json):
    if range == 'low':
        return fin_value(min(whatif_json[criteria]['shortTerm']['beforeTaxNetProfit']['low'],
                             whatif_json[criteria]['shortTerm']['beforeTaxNetProfit']['high']))
    else:
        return fin_value(max(whatif_json[criteria]['shortTerm']['beforeTaxNetProfit']['low'],
                             whatif_json[criteria]['shortTerm']['beforeTaxNetProfit']['high']))
...
def wordPopulate(template, out, property_address, pdfFileProperties, shortTerm, sensitivity, whatif_json):
    pageNumber = pdfFileProperties['pageStart']
    pagesTotalNumber = 1
    pdfFileProperties['tableContent'].update({pageNumber: 'What if Analysis – Vacation Rental Scenario'})
    ...
    cust_1 = {'ADR': property_address,
              ...
              'PPIN': lowValue(shortTerm['genericInput']['purchasePrice'], sensitivity, 'f'),
              'PP': fin_value(shortTerm['genericInput']['purchasePrice']),
              'PPAX': highValue(shortTerm['genericInput']['purchasePrice'], sensitivity, 'f'),
              'PPVI': whatif('purchasePrice', 'low', whatif_json),
              'PPV': fin_value(shortTerm['beforeTaxNetProfit']),
              'PPVX': whatif('purchasePrice', 'high', whatif_json),

              ## what if Rental Income
              'RMIN': lowValue(shortTerm['financialProjectionSummary']['income'][0] / 12, sensitivity, 'f'),
              'R': fin_value(shortTerm['financialProjectionSummary']['income'][0] / 12),
              'RMAX': highValue(shortTerm['financialProjectionSummary']['income'][0] / 12, sensitivity, 'f'),
              'RPVI': whatif('incomeAvoidedCosts', 'low', whatif_json),
              'RPVX': whatif('incomeAvoidedCosts', 'high', whatif_json),

              ## what if Opertaion Expense
              'OMIN': lowValue(shortTerm['financialProjectionSummary']['operationExpenses'][0] / -12, sensitivity, 'f'),
              'O': fin_value(shortTerm['financialProjectionSummary']['operationExpenses'][0] / -12),
              'OMAX': highValue(shortTerm['financialProjectionSummary']['operationExpenses'][0] / -12, sensitivity,
                                'f'),
              'OPVI': whatif('operationExpenses', 'low', whatif_json),
              'OPVX': whatif('operationExpenses', 'high', whatif_json),

              ## what if SELLING PRICE
              'SMIN': lowValue(shortTerm['taxProjection']['estimatedSalePrice'], sensitivity, 'f'),
              'S': fin_value(shortTerm['taxProjection']['estimatedSalePrice']),
              'SMAX': highValue(shortTerm['taxProjection']['estimatedSalePrice'], sensitivity, 'f'),
              'SPVI': whatif('SellPrice', 'low', whatif_json),
              'SPVX': whatif('SellPrice', 'high', whatif_json),

              ## what if Holding Period
              'HMIN': lowValue(shortTerm['genericInput']['holdingPeriodAnnual'] * 12, sensitivity, 't'),
              'H': str(round(shortTerm['genericInput']['holdingPeriodAnnual'])),
              'HMAX': highValue(shortTerm['genericInput']['holdingPeriodAnnual'] * 12, sensitivity, 't'),
              'HPVI': whatif('holdingPeriodAnnual', 'low', whatif_json),
              'HPVX': whatif('holdingPeriodAnnual', 'high', whatif_json),

              ## what if Buy&SellExpense
              'BMIN': lowValue(shortTerm['sellingExpense'] + shortTerm['purchaseExpense'], sensitivity, 'f'),
              'B': fin_value(shortTerm['sellingExpense'] + shortTerm['purchaseExpense']),
              'BMAX': highValue(shortTerm['sellingExpense'] + shortTerm['purchaseExpense'], sensitivity, 'f'),
              'BPVI': whatif('transActionCost', 'low', whatif_json),
              'BPVX': whatif('transActionCost', 'high', whatif_json),

              ## what if DownPayment
              'DMIN': lowValue(
                  shortTerm['genericInput']['downPaymentRate'] * shortTerm['genericInput']['purchasePrice'],
                  sensitivity, 'f'),
              'D': fin_value(shortTerm['genericInput']['downPaymentRate'] * shortTerm['genericInput']['purchasePrice']),
              'DMAX': highValue(
                  shortTerm['genericInput']['downPaymentRate'] * shortTerm['genericInput']['purchasePrice'],
                  sensitivity, 'f'),
    ...
    for item in scenarios:
        npv = shortTerm['beforeTaxNetProfit']
        lownpv = whatif_json[item]['shortTerm']['beforeTaxNetProfit']['low']
        highnpv = whatif_json[item]['shortTerm']['beforeTaxNetProfit']['high']
...
def main(pdfFileProperties):
    shortTerm, whatif_json = initialize()
    sensitivity = 0.15
    ...
    wordPopulate(f'{templateLocation}shortTermWhatIfTemplate', f'{outLocation}shortTermWhatIf{id}', property_address,
                 pdfFileProperties, shortTerm, sensitivity, whatif_json)
    word_to_pdf(f'{outLocation}shortTermWhatIf{id}')
    figuresOnPDF(f'{outLocation}shortTermWhatIf{id}')
    overlay(f'{outLocation}shortTermWhatIf{id}', f'{outLocation}shortTermWhatIf{id}.png',
            f'{outLocation}shortTermWhatIfOut{id}')
    maregePdf(f'{outLocation}shortTermWhatIfOut{id}', f'{outLocation}out{id}')
