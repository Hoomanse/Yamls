...
def wordPopulate(template, out, property_address, pdfFileProperties, shortTerm):
    ...
    pagesTotalNumber = 1
    pdfFileProperties['tableContent'].update({pageNumber: 'Financial Projection Summary – Vacation Rental'})
    ...
    for item in range(len(arrays)):
        ...
        oe[arrays[item]] = fin_value(shortTerm['financialProjectionSummary']['operationExpenses'][item])
        loan[arrays[item]] = fin_value(shortTerm['financialProjectionSummary']['loanPayments'][item])
        itax[arrays[item]] = fin_value(
            shortTerm["cashFlowProjection"]['cashFlowProjectionPurchased']['incomeTax'][item + 1])
        opcash[arrays[item]] = fin_value(shortTerm['financialProjectionSummary']['operationNetCashFlow'][item + 1])
    document.merge(PG=page_value(pageNumber),
                   ADR=property_address, HP=str(hp), IC=fin_value(shortTerm['initialCash'] * -1), YR1=yr[1], YR2=yr[2],
                   YR3=yr[3], YR4=yr[4], YR5=yr[5],
                   ...
                   NSP=fin_value(shortTerm["financialProjectionSummary"]['netSalePrice']),
                   LB=fin_value(-1 * shortTerm['mortgageBalanceRemaining']),
                   TDS=fin_value(shortTerm['taxProjection']['totalTaxDue'] * -1),
                   TSP=fin_value(shortTerm['afterTaxEquityReversion']),
                   ATP=fin_value(2 * round(shortTerm['beforeTaxNetProfit'] / 2000))
                   )
...
def figuresOnPDF(file, shortTerm):
    befortax = shortTerm["beforeTaxProfit"]
    aftertax = shortTerm["afterTaxProfit"]
    afterTax_profit = fin_value(2 * round(shortTerm['beforeTaxNetProfit'] / 2000))
    holding_period = shortTerm['genericInput']['holdingPeriodAnnual']
...
def main(pdfFileProperties):
    ...
    wordPopulate(f'{templateLocation}shortTermSummaryTemplate', f'{outLocation}shortTermSummary{id}', property_address,
                 pdfFileProperties, shortTerm)
    word_to_pdf(f'{outLocation}shortTermSummary{id}')
    figuresOnPDF(f'{outLocation}shortTermSummary{id}.png', shortTerm)
    overlay(f'{outLocation}shortTermSummary{id}', f'{outLocation}shortTermSummary{id}.png',
            f'{outLocation}shortTermSummaryOut{id}')
    maregePdf(f'{outLocation}shortTermSummaryOut{id}', f'{outLocation}out{id}')
