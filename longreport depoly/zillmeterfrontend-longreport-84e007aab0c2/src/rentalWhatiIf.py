...
def whatif(criteria, range, whatif_json):
    if range == 'low':
        return fin_value(min(whatif_json[criteria]['rental']['beforeTaxNetProfit']['low'],
                             whatif_json[criteria]['rental']['beforeTaxNetProfit']['high']))
    else:
        return fin_value(max(whatif_json[criteria]['rental']['beforeTaxNetProfit']['low'],
                             whatif_json[criteria]['rental']['beforeTaxNetProfit']['high']))
...
def wordPopulate(template, out, property_address, pdfFileProperties, rental, whatif_json, sensitivity):
    pageNumber = pdfFileProperties['pageStart']
    pagesTotalNumber = 1
    pdfFileProperties['tableContent'].update({pageNumber: 'What if Analysis – Rental Scenario'})
    ...
    cust_1 = {
        ...
        'PPIN': lowValue(rental['genericInput']['purchasePrice'], sensitivity, 'f'),
        'PP': fin_value(rental['genericInput']['purchasePrice']),
        'PPAX': highValue(rental['genericInput']['purchasePrice'], sensitivity, 'f'),
        'PPVI': whatif('purchasePrice', 'low', whatif_json),
        'PPV': fin_value(rental['beforeTaxNetProfit']),
        'PPVX': whatif('purchasePrice', 'high', whatif_json),

        ## what if Rental Income
        'RMIN': lowValue(rental['financialProjectionSummary']['income'][0] / 12, sensitivity, 'f'),
        'R': fin_value(rental['financialProjectionSummary']['income'][0] / 12),
        'RMAX': highValue(rental['financialProjectionSummary']['income'][0] / 12, sensitivity, 'f'),
        'RPVI': whatif('incomeAvoidedCosts', 'low', whatif_json),
        'RPVX': whatif('incomeAvoidedCosts', 'high', whatif_json),

        ## what if Opertaion Expense
        'OMIN': lowValue(rental['financialProjectionSummary']['operationExpenses'][0] / -12, sensitivity, 'f'),
        'O': fin_value(rental['financialProjectionSummary']['operationExpenses'][0] / -12),
        'OMAX': highValue(rental['financialProjectionSummary']['operationExpenses'][0] / -12, sensitivity, 'f'),
        'OPVI': whatif('operationExpenses', 'low', whatif_json),
        'OPVX': whatif('operationExpenses', 'high', whatif_json),

        ## what if SELLING PRICE
        'SMIN': lowValue(rental['taxProjection']['estimatedSalePrice'], sensitivity, 'f'),
        'S': fin_value(rental['taxProjection']['estimatedSalePrice']),
        'SMAX': highValue(rental['taxProjection']['estimatedSalePrice'], sensitivity, 'f'),
        'SPVI': whatif('SellPrice', 'low', whatif_json),
        'SPVX': whatif('SellPrice', 'high', whatif_json),

        ## what if Holding Period
        'HMIN': lowValue(rental['genericInput']['holdingPeriodAnnual'] * 12, sensitivity, 't'),
        'H': str(round(rental['genericInput']['holdingPeriodAnnual'])),
        'HMAX': highValue(rental['genericInput']['holdingPeriodAnnual'] * 12, sensitivity, 't'),
        'HPVI': whatif('holdingPeriodAnnual', 'low', whatif_json),
        'HPVX': whatif('holdingPeriodAnnual', 'high', whatif_json),

        ## what if Buy&SellExpense
        'BMIN': lowValue(rental['sellingExpense'] + rental['purchaseExpense'], sensitivity, 'f'),
        'B': fin_value(rental['sellingExpense'] + rental['purchaseExpense']),
        'BMAX': highValue(rental['sellingExpense'] + rental['purchaseExpense'], sensitivity, 'f'),
        'BPVI': whatif('transActionCost', 'low', whatif_json),
        'BPVX': whatif('transActionCost', 'high', whatif_json),

        ## what if DownPayment
        'DMIN': lowValue(rental['genericInput']['downPaymentRate'] * rental['genericInput']['purchasePrice'],
                         sensitivity, 'f'),
        'D': fin_value(rental['genericInput']['downPaymentRate'] * rental['genericInput']['purchasePrice']),
        'DMAX': highValue(rental['genericInput']['downPaymentRate'] * rental['genericInput']['purchasePrice'],
                          sensitivity, 'f'),
    ...
    for item in scenarios:
        npv = rental['beforeTaxNetProfit']
        lownpv = whatif_json[item]['rental']['beforeTaxNetProfit']['low']
        highnpv = whatif_json[item]['rental']['beforeTaxNetProfit']['high']
...
def main(pdfFileProperties):
    ...
    wordPopulate(f'{templateLocation}rentalWhatIfTemplate', f'{outLocation}rentalWhatIf{id}', property_address,
                 pdfFileProperties, rental, whatif_json, sensitivity)
    word_to_pdf(f'{outLocation}rentalWhatIf{id}')
    figuresOnPDF(f'{outLocation}rentalWhatIf{id}')
    overlay(f'{outLocation}rentalWhatIf{id}', f'{outLocation}rentalWhatIf{id}.png', f'{outLocation}rentalWhatIfOut{id}')
    maregePdf(f'{outLocation}rentalWhatIfOut{id}', f'{outLocation}out{id}')
